...
test.describe('Multiplayer - Complete Game Flow', () => {
	test('should complete full game flow with 2 players @e2e @multiplayer', async ({ browser }) => {
		...
		try {
			...
			const runButton1Visible = await game1.runButton.isVisible().catch(() => false);
			const runButton2Visible = await game2.runButton.isVisible().catch(() => false);
	...
	test('should show all players in rankings @e2e @multiplayer', async ({ browser }) => {
		const contexts: any[] = [];
		const pages: any[] = [];
		...
		try {
			...
			for (let i = 1; i <= 3; i++) {
				...
				await auth.register(generateTestUsername(`player${i}`), 'TestPassword123!');
			}

			// Host creates room
			const multiplayer1 = new MultiplayerPage(pages[0]);
			await multiplayer1.gotoMultiplayer();
			await pages[0].getByTestId(testIds.MULTIPLAYER_PAGE_BUTTON_HOST_ROOM).click();
			...
			for (let i = 1; i < pages.length; i++) {
				...
				await pages[i].getByTestId(testIds.MULTIPLAYER_PAGE_BUTTON_JOIN_ROOM).first().click();
				await pages[i].waitForTimeout(500);
			}

			// Start game
			await pages[0].getByTestId(testIds.MULTIPLAYER_PAGE_BUTTON_START_ROOM).click();

			// Wait for all to navigate to game
			await Promise.all(pages.map(page => 
				new MultiplayerPage(page).waitForGameStart()
			));
	...
	test('should handle player disconnection during game @e2e @multiplayer', async ({ browser }) => {
		const context1 = await browser.newContext();
		const context2 = await browser.newContext();
	...
	test('should support private room with invite code @e2e @multiplayer', async ({ browser }) => {
		const context1 = await browser.newContext();
		const context2 = await browser.newContext();
		...
		try {
			...
			const customGameButton = page1.getByTestId(testIds.MULTIPLAYER_PAGE_BUTTON_CUSTOM_GAME);
			if (await customGameButton.isVisible().catch(() => false)) {
				...
				const createButton = page1.locator('button:has-text("Create"), button:has-text("Host")').first();
