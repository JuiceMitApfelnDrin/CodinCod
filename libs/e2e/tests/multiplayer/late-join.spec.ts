...
test.describe("Multiplayer - Late Join Scenarios", () => {
	test("should allow joining during countdown @e2e @multiplayer", async ({
		browser,
	}) => {
	...
	test("should handle player leaving during countdown @e2e @multiplayer", async ({
		browser,
	}) => {
		const context1 = await browser.newContext();
		const context2 = await browser.newContext();
	...
	test("should handle host leaving during countdown @e2e @multiplayer", async ({
		browser,
	}) => {
		const context1 = await browser.newContext();
		const context2 = await browser.newContext();
		...
		try {
			...
			const startButton = page2.getByTestId(
