...
test.describe("Multiplayer - Chat System", () => {
	test("should send and receive messages in waiting room @e2e @multiplayer", async ({
		browser,
	}) => {
		...
		try {
			...
			const chat1 = page1.getByTestId(
				testIds.CHAT_COMPONENT_MESSAGES_CONTAINER,
			);
			const chat2 = page2.getByTestId(
				testIds.CHAT_COMPONENT_MESSAGES_CONTAINER,
			);
	...
	test("should support bidirectional chat in waiting room @e2e @multiplayer", async ({
		browser,
	}) => {
		const context1 = await browser.newContext();
		const context2 = await browser.newContext();
	...
	test("should handle chat with 3+ players @e2e @multiplayer", async ({
		browser,
	}) => {
		const context1 = await browser.newContext();
		const context2 = await browser.newContext();
		...
		try {
			...
			const chat1 = await page1
				.getByTestId(testIds.CHAT_COMPONENT_MESSAGES_CONTAINER)
				.textContent();
			const chat2 = await page2
				.getByTestId(testIds.CHAT_COMPONENT_MESSAGES_CONTAINER)
				.textContent();
			const chat3 = await page3
				.getByTestId(testIds.CHAT_COMPONENT_MESSAGES_CONTAINER)
				.textContent();
