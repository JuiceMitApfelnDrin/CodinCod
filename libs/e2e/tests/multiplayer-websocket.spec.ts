...
test.describe('Multiplayer - WebSocket Resilience', () => {
	test('should handle multiple players connecting and disconnecting @e2e @multiplayer', async ({ browser }) => {
		const contexts: any[] = [];
		const pages: any[] = [];
		...
		try {
			...
			for (let i = 2; i <= 5; i++) {
				...
				await page.getByTestId(testIds.MULTIPLAYER_PAGE_BUTTON_JOIN_ROOM).first().click();
	...
	test('should maintain room state after temporary disconnection @e2e @multiplayer', async ({ browser }) => {
		const context1 = await browser.newContext();
		...
		try {
			...
			const multiplayer1 = new MultiplayerPage(page1);
			await auth1.gotoRegister();
			const hostUsername = generateTestUsername('host');
			...
			// Verify 2 players
			let players = await multiplayer1.getPlayersInRoom();
	...
	test('should handle rapid connection/disconnection @e2e @multiplayer', async ({ browser }) => {
		const context1 = await browser.newContext();
		const context2 = await browser.newContext();
	...
	test('should handle room cleanup when all players disconnect @e2e @multiplayer', async ({ browser }) => {
		const context1 = await browser.newContext();
		const context2 = await browser.newContext();
		...
		try {
			...
			await auth3.register(generateTestUsername('observer'), 'TestPassword123!');
			await multiplayer3.gotoMultiplayer();
			await page3.waitForTimeout(1000);

			const roomsBeforeCleanup = await page3.getByTestId(testIds.MULTIPLAYER_PAGE_BUTTON_JOIN_ROOM).count();
			...
			const roomsAfterCleanup = await page3.getByTestId(testIds.MULTIPLAYER_PAGE_BUTTON_JOIN_ROOM).count();
