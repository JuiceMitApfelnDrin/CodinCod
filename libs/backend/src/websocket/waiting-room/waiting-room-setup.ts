...
export function waitingRoomSetup(
    ...
    switch (event) {
      ...
      case waitingRoomEventEnum.START_GAME: {
        try {
          ...
          console.info(`Game ${newlyCreatedGame.id} started with ${players.length} players`);
        } catch (error) {
  ...
  socket.on("close", (code, reason) => {
    ...
    console.info(`Waiting room socket closed for ${req.user.username}: ${code} - ${reason}`);
  ...
  socket.on("error", (error) => {
    ...
    fastify.log.error(`Waiting room socket error for ${req.user.username}:`, error);
    waitingRoom.removeUserFromUsers(req.user.username);
    waitingRoom.removeEmptyRooms();
  });
}
