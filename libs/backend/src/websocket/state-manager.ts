...
export async function setUserState(username: string, state: UserState): Promise<void> {
	const redis = getRedis();
	await redis.hset(`${WEBSOCKET_CONSTANTS.REDIS.KEYS.USER_PREFIX}${username}`, state);
}

export async function getUserState(username: string): Promise<UserState | null> {
	const redis = getRedis();
	const data = await redis.hgetall(`${WEBSOCKET_CONSTANTS.REDIS.KEYS.USER_PREFIX}${username}`);
...
export async function setRoomState(roomId: string, state: RoomState): Promise<void> {
	const redis = getRedis();
...
export async function getRoomState(roomId: string): Promise<RoomState | null> {
	const redis = getRedis();
	const data = await redis.hgetall(`${WEBSOCKET_CONSTANTS.REDIS.KEYS.ROOM_PREFIX}${roomId}`);
...
export async function getAllRoomStates(): Promise<RoomState[]> {
	const redis = getRedis();
	const keys = await redis.keys(`${WEBSOCKET_CONSTANTS.REDIS.KEYS.ROOM_PREFIX}*`);
...
export async function removeUserFromRoom(username: string, roomId: string): Promise<boolean> {
	const room = await getRoomState(roomId);
	const userState = await getUserState(username);
	...
	room.playerUsernames = room.playerUsernames.filter(name => name !== username);
