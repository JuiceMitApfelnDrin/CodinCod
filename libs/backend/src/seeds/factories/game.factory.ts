...
/**
 * Generate game allowed languages
 */
function generateAllowedLanguages() {
	const allLanguages = [
		{ language: "python", version: "3.10.0" },
		{ language: "javascript", version: "18.15.0" },
		{ language: "java", version: "15.0.2" },
		{ language: "cpp", version: "10.2.0" },
		{ language: "rust", version: "1.68.0" },
		{ language: "go", version: "1.20.0" }
	];

	// Select 1-4 languages
	const count = faker.number.int({ min: 1, max: 4 });
	return randomMultipleFromArray(allLanguages, count, count);
}
...
export async function createGame(
	...
	const gameData: Partial<GameDocument> = {
		...
		options: {
			...
			allowedLanguages: generateAllowedLanguages()
		},
		playerSubmissions: []
	};
