<script lang="ts">
	...
	import { PuzzleResultEnum, type ValidatorEntity } from "types";

	let className: HTMLInputAttributes["class"] = undefined;
	export { className as class };

	export let validator: ValidatorEntity;
</script>

{#if validator.testResult?.run.result === PuzzleResultEnum.SUCCESS}
	<span class="sr-only">successful test</span>
	<CheckCircle
		class={cn("text-green-700 dark:border-green-300 dark:text-green-500", className)}
		aria-hidden="true"
	/>
{:else if validator.testResult?.run.result === PuzzleResultEnum.ERROR || validator.testResult?.run.result === PuzzleResultEnum.UNKNOWN}
